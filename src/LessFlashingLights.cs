...
public partial class LessFlashingLights : Mod, ITogglableMod, IGlobalSettings<GlobalSettings>, ICustomMenuMod
{
    ...
    private static bool _inGrimmFight;//For disabling an FSM used in other places which breaks stuff when disabled outside the fight
    private static bool _inHornetFight;//for both of them
    private static bool _inShadeSoulPickup;//Again, to only disable an FSM if we know what we're doing
    private static bool _ghostExploding;//I do not remember what was the logic behind this, but I'm too scared to remove it
